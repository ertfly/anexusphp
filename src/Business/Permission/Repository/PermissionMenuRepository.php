...
class PermissionMenuRepository
{
    ...
    {
        $db = Database::getInstance();
        $regs = $db->query('select * from ' . PermissionMenuEntity::TABLE . ' where category_id = :category_id', ['category_id' => (int)$id])->fetchAll(PDO::FETCH_CLASS, PermissionMenuEntity::class);
